/*
  insert_game: aceitar end_date (date, opcional) para eventos de vários dias.
*/
...
BEGIN
  ...
  INSERT INTO public.games (
    jornada, opponent, location, phase, game_type, created_by, team_id, starts_at, end_date, status
  )
  VALUES (
    p_round_number,
    ...
    CASE WHEN p_phase IN ('Qualificação', 'Regionais', 'Nacionais') THEN 'Liga'::text ELSE COALESCE(nullif(trim(p_phase), ''), 'Liga')::text END,
    v_created_by,
    v_team_id,
